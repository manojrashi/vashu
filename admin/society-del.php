...
if(count($arr)>0){
	...
	{	
	    ...
	    $sql="delete from $tbl_society where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_society set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_society set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
