...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{
		...
		$imgArr=$obj->query("select photo from $tbl_topic where id in ($str_rest_refs) ");	
		while($resultImage=$obj->fetchNextObject($imgArr)){
		@unlink("../upload_images/topic/".$resultImage->photo);
		@unlink("../upload_images/topic/thumb/".$resultImage->photo);
		}	
	    $sql="delete from $tbl_topic where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_topic set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_topic set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
