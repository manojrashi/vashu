...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{
		...
		$imageArr=$obj->query("select logo from  $tbl_brand where id in ($str_rest_refs) ");
	    ...
	    $sql="delete from $tbl_brand where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_brand set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_brand set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
