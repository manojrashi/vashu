...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{
		$sql="delete from $tbl_unit where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_unit set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_unit set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
