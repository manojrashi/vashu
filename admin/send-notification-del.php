...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{  
	    $sql="delete from $tbl_schedule_notification where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_schedule_notification set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_schedule_notification set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
