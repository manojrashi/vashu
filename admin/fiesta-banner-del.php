...
if(count($arr)>0){
...

	{
	     ...
	     $imageArr=$obj->query("select photo from $tbl_fiesta_banner where id  in ($str_rest_refs)");
	    ...
	    $sql="delete from $tbl_fiesta_banner where id in ($str_rest_refs)"; 
	...
	{	
		...
		$sql="update $tbl_fiesta_banner set status=1 where id in ($str_rest_refs)";
	...
	{		
		 ...
		 $sql="update $tbl_fiesta_banner set status=0 where id in ($str_rest_refs)";
