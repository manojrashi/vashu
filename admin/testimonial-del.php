...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{
		 $imageArr=$obj->query("select photo from $tbl_testimonial where id  in ($str_rest_refs)");
	    ...
	    $sql="delete from $tbl_testimonial where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_testimonial set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_testimonial set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
