...
if(count($arr)>0){
	...
	{	
	    ...
	    $sql="delete from $tbl_faqcat where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_faqcat set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_faqcat set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
