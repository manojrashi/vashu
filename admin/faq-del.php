...
if(count($arr)>0){
	...
	{	  
	    ...
	    $sql="delete from $tbl_faq where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_faq set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_faq set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
