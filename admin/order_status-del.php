...
if(count($arr)>0){
	...
	{	  
	    ...
	    $sql="delete from $tbl_order_status where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_order_status set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		$sql="update $tbl_order_status set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
