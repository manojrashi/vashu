...
$sql=$obj->query("select count(id) as num_rows from $tbl_order where 1=1 and order_status=4",$debug=-1);
$line=$obj->fetchNextObject($sql);
$totalData=$line->num_rows; //die;
$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.

$sql = "select a.* from $tbl_order as a INNER JOIN $tbl_user as b ON a.user_id=b.id where 1=1 and a.order_status=4";
