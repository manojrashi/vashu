...
class Cl_User
{
	...
	public function registration( array $data )
	{
		if( !empty( $data ) ){
			...
			$query = "INSERT INTO tbl_user (user_id, name, email, password, created) VALUES (NULL, '$name', '$email', '$password', CURRENT_TIMESTAMP,1)";
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "SELECT user_id, name, email, created FROM tbl_user where email = '$email' and password = '$password' ";
	...
	public function fb_login( array $data )
	{
		...
		$query = "SELECT user_id, name, email, created FROM tbl_user where email = '$email' and social_id = '$social_id' ";
		$result = mysqli_query($this->_con, $query);
		$data = mysqli_fetch_assoc($result);
		$count = mysqli_num_rows($result);
		if( $count >= 1){
			...
			$query = "INSERT INTO tbl_users (user_id, password, name, email, social_id, created) VALUES (NULL, '$password1', '$name', '$email', '$social_id', CURRENT_TIMESTAMP)";
	...
	public function google_login( array $data )
	{
	  ...
	  $query = "SELECT user_id, name, email, created FROM tbl_users where email = '$email' and social_id = '$social_id' ";
		$result = mysqli_query($this->_con, $query);
		$data = mysqli_fetch_assoc($result);
		$count = mysqli_num_rows($result);
		if( $count == 1){
			...
			$password = $this->randomPassword();
			$password1 = ( $password );			
			$query = "INSERT INTO tbl_users (user_id,password, name, email, social_id, picture, created) VALUES (NULL, '$password1','$name', '$email', '$social_id', '$picture', CURRENT_TIMESTAMP)";

		//echo $query; 
			
			if(mysqli_query($this->_con, $query)){
				$to = $email;
				$subject = "Account Create - Rextie";
				$txt = "Your Email ".$email;
				$txt .= "<br/>Your New Password ".$password;
				$headers = "From: arvinduniquecoder@gmail.com" . "\r\n" .
						"CC: arvinduniquecoder@gmail.com";
	...
	public function twitter_login( array $data )
	{
		...
		$query = "SELECT user_id, name, email, created FROM tbl_user where social_id = '$social_id' ";
		$result = mysqli_query($this->_con, $query);
		$data = mysqli_fetch_assoc($result);
		$count = mysqli_num_rows($result);
		if( $count == 1){
			...
			$query = "INSERT INTO tbl_user (user_id, password, name, social_id, picture, created) VALUES (NULL, '$password1', '$name', '$social_id', '$picture', CURRENT_TIMESTAMP)";
			if(mysqli_query($this->_con, $query));
			$query = "SELECT user_id, name, email, created FROM tbl_user where social_id = '$social_id' ";
			$result = mysqli_query($this->_con, $query);
			$data = mysqli_fetch_assoc($result);
	...
	public function forgetPassword( array $data )
	{
		if( !empty( $data ) ){
			...
			$password = $this->randomPassword();
			$password1 = md5( $password );
			$query = "UPDATE tbl_user SET password = '$password1' WHERE email = '$email'";
			if(mysqli_query($this->_con, $query)){
				$to = $email;
