...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{
	   $sql="delete from $tbl_wishlist where id in ($str_rest_refs)"; 
...
// die;
	...
	{	
		$sql="update $tbl_wishlist set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_wishlist set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
